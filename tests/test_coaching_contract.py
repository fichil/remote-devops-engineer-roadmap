...
def test_coach_contract_forbids_questionnaire_and_copy_chain() -> None:
    ...
    assert "require the fixed four-part" in contract
    ...
    assert "Ask at most one meaningful decision question" in contract
...
def test_automation_prompt_carries_cognitive_apprenticeship_rules() -> None:
    ...
    assert "认知学徒制任务按讲解与示范、引导练习、独立迁移与交付推进" in automation
    assert "不得固定要求“事实、风险、缺失信息、假设”四项" in automation
    assert "只贴输出或只报退出码不能证明" in automation
