...
- `继续处理遗留`: only after today's primary task and every previously completed carryover have complete evidence and completed automatic publications, run `python -m devops_coach today --continue-carryover`. Teach the first unfinished checkpoint of the oldest carryover. Each explicit request starts only one carryover; after its separate publication, the learner may explicitly repeat the request without a daily carryover limit.
