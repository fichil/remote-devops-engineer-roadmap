...
- `继续处理遗留`: recover every earlier unfinished publication first. On weekdays, today's primary must also be complete and published; weekends require no primary. Run `python -m devops_coach today --continue-carryover` to start or resume the oldest existing carryover. Teach its first unfinished checkpoint. Each explicit request starts only one carryover; after its separate automatic publication, stop. Another explicit request may start the next one without a daily carryover limit.
...
- After today's primary task is complete, stop by default. Start only the oldest carryover when the learner explicitly asks to continue. After that carryover is complete and separately published, stop again; another explicit request may start the next oldest carryover. Do not impose a daily carryover count limit.
- Saturday and Sunday have zero required quota. Ordinary `today` is read-only and never generates, reschedules, or backfills routine work. Explicit carryover requests may use existing tasks and their original private weekly lab.
- Record the real evidence-completion date, preserving `scheduled_for`. `record --date YYYY-MM-DD` supports delayed entry of existing evidence and rejects future dates. Completion logs, retrieval dates, and publication identities use that date. Weekend carryovers count toward completed tasks, never toward weekday streaks or missed Friday credit. A retrieval date on a weekend is a due marker, not a new weekend task.
...
An evidence-complete weekday primary task has durable authorization to publish automatically. An explicitly activated, evidence-complete carryover has a separate automatic publication, including on weekends without a same-day primary. Partial checkpoints, unactivated carryovers, weekend primary tasks, unverified evidence, or unrelated changes never authorize publication. Recover older pending publications before recording or publishing the next carryover; recovery retains the original completion date and publication key.
...
5. Publish the primary as `learn/YYYY-MM-DD`. Publish each carryover on its own deterministic `learn/YYYY-MM-DD-carryover-<task-token>` branch and publication key. Create one Ready PR per task, wait for CI, squash merge the exact head, then synchronize `main` and delete only the merged publication branch.
