...
- 时间：工作日每天 75 分钟，周六和周日完全休息，每周约 6.25 小时。
...
- 每个工作日只生成一个真实运维情境任务；重复启动会恢复原计划，周末不生成计划。
...
- 显示非惩罚性工作日参与连胜：有证据且至少投入 15 分钟的 `partial` 或 `done`
  可以计入；周末跳过，连胜不改变评分或阶段门禁。
- 英语至少占学习时间的 25%，并逐步从单词、短句过渡到英文 README、Issue、事故复盘、演示和面试。
- 日常英语只评价阅读和写作；通用口语与发音练习转到 Duolingo，路线中明确安排的技术演示和模拟面试仍然保留。
...
| 日期 | 事件简报 | 实战处理 | 英文书面交接 | 总时长 |
|---|---:|---:|---:|---:|
| 周一至周五 | 15 分钟 | 40 分钟 | 20 分钟 | 75 分钟 |
| 周六、周日 | — | — | — | 完全休息 |

减载任务为 60 分钟（10/35/15），加速任务为 80 分钟（15/45/20）。忙碌工作日只要
提交证据并实际投入至少 15 分钟，就可以记录为 `partial`；第五个工作日任务集成周复盘。
...
每个阶段都必须交付一个能运行的公开项目、中英双语说明和英文演示。完整周主题、前四周
情境任务卡和后续五日轮换位于 [`curriculum/roadmap.yml`](curriculum/roadmap.yml)。证据不足时
延长路线，不为维持名义上的 78 周日期而降低门禁。
...
个人环境只保留一条北京时间工作日 09:00 的本地定时任务；周末没有自动化，也不自动补课。
任务可以评价英语阅读和写作，但不追踪通用口语、发音或录音练习。电脑未开导致错过时可
手动运行，不需要 OpenAI API Key。配置和手动试跑方法见 [Codex 自动化说明](docs/codex-automation.md)。
